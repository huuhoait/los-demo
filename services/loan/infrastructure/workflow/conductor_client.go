...
// updateTaskViaREST updates a task using the REST API as a fallback
func (c *ConductorClientImpl) updateTaskViaREST(
	ctx context.Context,
	workflowInstanceId string,
	referenceTaskName string,
	status string,
	...
	logger := c.logger.With(
		...
		zap.String("reference_task_name", referenceTaskName),
		zap.String("status", status),
		zap.String("operation", "update_task_rest"),
	)
	...
	endpoints := []string{
		// Endpoint 1: Standard task update endpoint
		fmt.Sprintf("%s/api/tasks/%s/%s/%s", c.baseURL, workflowInstanceId, referenceTaskName, status),
		// Endpoint 2: Workflow-based task update
		fmt.Sprintf("%s/api/workflow/%s/tasks/%s", c.baseURL, workflowInstanceId, referenceTaskName),
	}

	for i, endpoint := range endpoints {
		logger.Debug("Trying REST API endpoint", zap.Int("endpoint_index", i), zap.String("endpoint", endpoint))
		...
		if workflowInstanceId != "" {
			requestBody["workflowInstanceId"] = workflowInstanceId
		}
		if referenceTaskName != "" {
			requestBody["referenceTaskName"] = referenceTaskName
		}
		...
		// Check if successful
		if resp.StatusCode >= 200 && resp.StatusCode < 300 {
			logger.Debug("Task updated successfully via REST API", zap.Int("endpoint_index", i))
			return nil
		}

		logger.Warn("REST API endpoint failed",
			zap.Int("endpoint_index", i),
			zap.String("endpoint", endpoint),
			zap.Int("status_code", resp.StatusCode),
			zap.String("response", string(responseBody)))
	...
	return fmt.Errorf("all task update methods (SDK and REST) failed")
}
